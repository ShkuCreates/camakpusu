    ...
    <article className="glass-panel campus-hover rounded-[26px] p-5">
      <div className="mb-3 flex items-start justify-between gap-3">
        <div>
          ...
          <h3 className="mt-2 text-xl font-semibold text-zinc-900">{task.title}</h3>
        </div>
        <span className="sticker-tag border-lime-300/80 bg-[#f0ffad]/70 text-[#4b6015]">
          {task.status}
        </span>
      </div>
      ...
      <div className="mt-5 flex items-end justify-between gap-4 border-t border-zinc-200/80 pt-4">
        <div>
      ...
      <div className="mt-5 flex items-center justify-between">
        ...
        <Link
          href={`/task/${task.id}`}
          className="inline-flex items-center justify-center rounded-full border border-zinc-900 bg-zinc-900 px-4 py-2 text-sm font-medium text-white transition hover:bg-zinc-700"
        >
          View task
        </Link>
      </div>
